...
\bigskip\\Solution:\\
    ...
    Let us define $Y_2$ and $X_3$ as the random variables for the 2\textsuperscript{nd} and 3\textsuperscript{rd} dice roll, respectively. Let $Y$ denote the random variable for the combined triple dice roll.\\
    Now, we can write $Y=Y_1+Y_2+Y_3$.\\
...
\end{minipage}
    ...
    \\
    \item
    calculating the roots by newton-raphson method(Ref\eqref{eq:raph})\\
...
\begin{align}
    ...
    sG\brak{s}&=\mathcal{L}\{y'\} \tag{Refer \eqref{eq:laplace1d}}
\end{align}
...
\end{align}
\item
we use forward euler method (Refer to \eqref{eq:fwdeuler})to find the graph of the given function:\\
The difference function of forward euler becomes:\\
