...
\solution
We use Euler's theorem for homogeneous functions.(see \eqref{eq:eulerhomo})
...
Since we know that the given equation is homogeneous and of degree 4, we can apply this theorem. (see \eqref{eq:eulerhomo})
\begin{align}\
...
\begin{align}
    y&\system{L}G\brak{s}
\end{align}
From \eqref{eq:laplace1d}
\begin{align}
    y'&\system{L}sG\brak{s} \\
...
\begin{align}
   y&\system{L}G\brak{s}
  \end{align}
From \eqref{eq:laplacee^ax},we can write
\begin{align}
                 y&=e^{3x}-e^{x} 
\end{align}
