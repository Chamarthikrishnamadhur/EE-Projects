\begin{enumerate}[label=\thesubsection.\arabic*,ref=\thesubsection.\theenumi]


 \item A fair six-faced dice, with the faces labelled ‘1’, ‘2’, ‘3’, ‘4’, ‘5’, and ‘6’, is rolled
    thrice. What is the probability of rolling ‘6’ exactly once?\hfill(GATE AG 2025)
 \begin{enumerate}
    \item $\frac{75}{216}$
    \item $\frac{1}{6}$
    \item $\frac{1}{18}$
    \item $\frac{25}{216}$
\end{enumerate}
\solution
     The sample space is S $\in \{1,2,3,4,5,6\}$.
     Let us define a variable $Y_1$ as follows: 
     \begin{align}
         Y_1=
            \begin{cases}
                1&X_1=6\\
                0&X_1\neq6
            \end{cases}
     \end{align}
     Now, The PMF is defined as:
        \begin{align}
            P_{Y_1}(k)=
                \begin{cases}
                    \frac{1}{6} &  k=1 \\
                    \frac{5}{6} & k=0\\
                \end{cases}\\\\
            \text{Let $p,q$ be $\frac{1}{6}$ and $\frac{5}{6}$ respectively.}
        \end{align}
        
    %\noindent Step 2: Applying Z-Transform to find the required PMF for Y \\\\
    Let us define $Y_2$ and $X_3$ as the random variables for the 2\textsuperscript{nd} and 3\textsuperscript{rd} dice roll, respectively. Let $Y$ denote the random variable for the combined triple dice roll.We write
\begin{align}
        Y=Y_1+Y_2+Y_3
\end{align}
    Writing the Z-Transform (see \eqref{eq:ztransform}) for $Y$
\begin{align}
	M_{Y_1}(Z)&=\brak{P_{Y_1}(1)Z^{-1} +P_{Y_1}(0)Z^{-0}}\\
    				&=Z^{-1}+q
        \end{align}
    We know that $Y_1,Y_2,Y_3$ are independent events. From 
\eqref{eq:ztransformindep}
    \begin{align}
    M_Y\brak{Z}&=M_{Y_1}\brak{Z}\,\, M_{Y_2}\brak{Z}\,\,M_{Y_3}\brak{Z}\\
        M_Y\brak{Z}&=\brak{pZ^{-1}+q}^3\\
              &=\sum_{k=0}^3{^3}C_k\,\,q^{3-k}\,\, \brak{pZ^{-1}}^k
     \end{align} 
     From \eqref{eq:ztransformdef}
     \begin{align}      
              =\sum P_Y\brak{k}Z^{-k}
              \end{align}
So, we can say:        
        \begin{align}
            P_Y\brak{k}&=^3C_k\,\,q^{3-k}\,\,p^k
    \end{align}
    The above situation is called as a binomial distribution.
    %\noindent Step 3: Using binomial distribution to get required probability
    We need the situation where $p$ appears exactly once. So, we consider only the term where the power of $p$ is 1.
    The required term is :  $^3C_1\,\,p\,\,q^2$.
    Substituting the values of $p$ and $q$, we get:
    \begin{align}
        ^3C_1\,\frac{1}{6}\,\,\brak{\frac{5}{6}}^2
        =\frac{75}{216}
    \end{align}
    Therefore, the required answer is $\frac{75}{216}$.
The following code simulates the result :
\begin{lstlisting}
codes/Probability.c
\end{lstlisting}






	\item If a, b, and c are the roots of $2x^3 - 3x^2 + px - 1=0$ and sum of the two roots is 1, the value of $p$ is:\hfill(GATE AG 2025)
\begin{enumerate}
    \item 2
    \item 1
    \item 3
    \item $\frac{1}{3}$
\end{enumerate}

    \solution
    \begin{enumerate}
  	\item
    Let the roots whose sum is 1 be $a$ and $b$.
    Then, by using the properties of coefficients of an equation, we write:
    \begin{align}
        -\brak{\frac{-3}{2}}&=a+b+c\\
                               \frac{3}{2} &=1+c\\
                               c&=\frac{1}{2} \label{eq:valofc}
    \end{align}
    And
    \begin{align}
        abc&=-\brak{\frac{-1}{2}}\\
        ab\,\frac{1}{2}&=\frac{1}{2}
    \end{align}
        From \eqref{eq:valofc},
    \begin{align}         
        ab&=1\label{eq:valofab}
    \end{align}
    Also,
    \begin{align}
        ab+bc+ac&=\brak{\frac{p}{2}}
    \end{align}
       From \eqref{eq:valofab} and \eqref{eq:valofc},we write
    \begin{align}
        1+c(a+b)&=\frac{p}{2}\\
        1+\frac{1}{2}(1)&=\frac{p}{2}
        3=p
    \end{align}
    Therefore, the value of $p$ is 3.
    \item
    Calculating the roots by newton-raphson method (see \eqref{eq:raph}).
    The given cubic equation is :$2x^3-3x^2+3x-1$.
    Therefore, the difference equation becomes:
    \begin{align}
    x_{n+1}=x_n+\frac{2x^3-3x^2+3x-1}{6x^2-6x+3}   
    \end{align}

    With an initial guess of 4, running this difference equation 6 times gives us the real root, which is $\frac{1}{2}$
    \begin{figure}[htbp]
    \centering
    \includegraphics[width=0.7\textwidth]{figs/rson.png}
    \caption{The depiction of newton raphson method guessing the root}
    \label{Newtonraphsonfig}
\end{figure}

\end{enumerate}
The following code verifies the result:
\begin{lstlisting}
codes/raph.py
\end{lstlisting}







\item If the matrix $\myvec{ x & -3 \\ 2 & x-5 }$ is singular, sum of the values of $x$ is:     \hfill(GATE AG 2025)   
    \begin{enumerate}
    \item 5
    \item 6
    \item  0
    \item $-1$
\end{enumerate}
\solution
A matrix is said to be singular if its row echelon form has a row full of zeroes.
Finding row echelon form of the given matrix:
\begin{align*}
    \myvec{
        x& -3 \\
        2& x-5
    }
    \xleftrightarrow{R_2 \to \frac{R_2}{2}}
    \myvec{
        x&-3\\
        1&\frac{(x-5)}{2} 
         } 
    \xleftrightarrow{R_1 \to R_1 - xR_2}\\
    \myvec{
        x-x\cdot1&\frac{-6-(x-5)}{2}\\
        1&\frac{\brak{x-5}}{2} 
        }
	\xleftrightarrow{R_2 \to R_1  R_2 \to R_1}
	\myvec{
        1&\frac{\brak{x-5}}{2}\\
        0&\frac{\brak{-6-x^2+5x}}{2}
        }    
\end{align*}
%\begin{center}
 %     $\myvec{
  %      x&-3\\
  %      2&x-5 
  %     }$\bigskip\\
  %      $\myvec{
  %      x-x\cdot1&\frac{-6-(x-5)}{2}\\
  %      1&\frac{\brak{x-5}}{2} 
  %     }$\bigskip\\
  %      $\myvec{
  %      0&\frac{\brak{-6-x^2+5x}}{2}\\
  %      1&\frac{\brak{x-5}}{2} 
  %      }$\bigskip\\
  %      $\myvec{
  %      1&\frac{\brak{x-5}}{2}\\
  %      0&\frac{\brak{-6-x^2+5x}}{2}
  %      }$\bigskip\\  
%\end{center}
%\noindent Step 2: Making the bottom row to be full of zeroes:\medskip\\
Equating $\frac{\brak{-6-x^2+5x}}{2}$ to zero gives us: 
\begin{align}
    -6-x^2+5x=0\\
    x^2-5x+6=0
\end{align}
So, the sum of values of $x$ is 5.
The following code converts a numerical matrix into its row echelon form.So,
we can verify that the values of ($x=2,3$) give us a matrix with a row of zeroes
The following code verifies the result:
\begin{lstlisting}
codes/echelon.py
\end{lstlisting}






\item $\frac{s}{s^2+a^2}$ is the Laplace transform of:\hfill(GATE AG 2025)
    \begin{enumerate}
    \item cos$\brak{at}$
    \item sin$\brak{at}$
    \item sinh$\brak{at}$
    \item cosh$\brak{at}$
                
    \end{enumerate}

    
\solution
From \eqref{eq:laplacecommon}, 
\begin{align}
\cos\brak{at}\system{L}\frac{a}{s^2+a^2}
\end{align}
%no options





\item The median of the following set of numbers: 154, 130, 144, 137, 156, 146, 138,
149, 160, 138 is:\hfill(GATE AG 2025)
    \begin{enumerate}   
    \item[(A)] $145.0$
    \item[(B)] $145.2$
    \item[(C)] $151.0$
    \item[(D)] $146.0$
    %\item[SOL] NEED TO WRITE SOL!!!
    \end{enumerate}   

\solution
To find the median, we need to arrange the data in ascending or descending order.
After arranging them in ascending order, the data is as follows
\begin{align}
130,137,138,138,144,146,149,154,156,160
\end{align}
The number of elements in the data set are 10.
Since it is an even number, the median ($M$) is defined as the average of the middle 2 terms (in this case, average of $5^{th}$ and $6^{th}$ terms).
Therefore, the median($M$) is $\frac{144+146}{2} = 145.0$.
The following codes verify the result:
\begin{lstlisting}
codes/Sorter.py
codes/Quicksort.c
\end{lstlisting}






\item The series \\ $\sum_{n=0}^{r} q^n$ $=1+q+q^2+... ... ...$ has the sum:\hfill(GATE AG 2025)
    \begin{enumerate}
    \item $\frac{1}{1-q}$-$\frac{q^{r+1}}{1-q}$
    \item $\frac{1}{q-1}$+$\frac{q^{r+1}}{q-1}$
    \item $\frac{1}{1-q}$+$\frac{q^{r+1}}{1-q}$
    \item $\frac{1}{q-1}$
    %\item[SOL] NEED TO WRITE SOL!!! 
    \end{enumerate}

\solution
    The given sequence is a geometric progression with initial term as 1 and common ratio as $q$ and the number of terms are $r+1$.(The series converges only if $q<1$).
    Applying formula \eqref{eq:sumofgp}, we get :
    \begin{align}
        &1\,\frac{q^{r+1}-1}{q-1}\\
        &=\frac{1-q^{r+1}}{1-q}\\
        &=\frac{1}{1-q}-\frac{q^{r+1}}{1-q}        
    \end{align}
Therefore, the answer is option A.
The following code verifies the result:
\begin{lstlisting}
codes/gp.py
\end{lstlisting}
  





\item The value of $\lim_{x\to 0}\frac{x\cos\brak{x}-\sin\brak{x}}{x^2\sin\brak{x}}$\hfill(GATE AG 2025)
    \begin{enumerate}
        \item $\frac{-1}{3}$
        \item $\frac{1}{3}$
        \item $3$
        \item$-3$
        
\end{enumerate}


    
        \solution
        We use the idea of taylor series expansion (converges if $x<1$) to solve the problem.\\
        
       \begin{align}
        &\frac{x\brak{\cos\brak{x}}-\sin\brak{x}}{x^2\sin\brak{x}}\\
        \end{align}
        From \eqref{eq:expansions}
        \begin{align}
        &=\frac{x(1-\frac{x^2}{2!} .....)-(x-\frac{x^3}{3!}.....)}{x^2(x-\frac{x^3}{3!}.....)}
        \end{align}
        {When $x$ approaches zero, we can ignore the higher power terms.}\\
        \begin{align}
    		&=\frac{(x-\frac{x^3}{2})-(x-\frac{x^3}{6})}{x^3(1-\frac{x^2}{6})}\\
        &=\frac{\frac{-1}{3}}{1-\frac{x^2}{6}}
       \end{align}
       {Substituting $x$ as zero gives us the limit as $\frac{-1}{3}$.}
        
        

The following code simulates the result:
\begin{lstlisting}
codes/Plottinglimit.py
\end{lstlisting}

 The output graph is shown below:\\
\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.7\textwidth]{figs/limitplot.png}
    \caption{The plot depicting the approaching of the limit to $\frac{-1}{3}$}
    \label{Plot of the function approaching the limit}
\end{figure}






\item If $u=x^4+y^4+3x^2y^2$, the value of $\left(x\frac{\partial{u}}{\partial{x}}+y\frac{\partial{u}}{\partial{y}}\right)^{-1}$\hfill(GATE AG 2025)
\begin{enumerate}
    \item $4u$
    \item $0$
    \item $\frac{1}{4u}$
    \item $2u$
    %\item[SOL] NEED TO WRITE SOL!!! 
\end{enumerate}


\solution
We use Euler's theorem for homogeneous functions.(Refer to \eqref{eq:eulerhomo} for the theorem)
%Step 1: Proving the given function is homogeneous:\medskip 
\begin{align}
    u=g\brak{x,y}&=x^4+y^4+3x^2y^2\\
g\brak{\alpha x,\alpha y}&=\alpha^4\brak{x^4+y^4+3x^2y^2}  
\end{align}
Therefore, u is a homogeneous function of degree 4.
%Step 2: Using Euler's theorem to get the answer\medskip\\
Since we know that the given equation is homogeneous and of degree 4, we can apply this theorem.
\begin{align}\
    x\frac{\partial u}{\partial x}+y\frac{\partial u}{\partial y}&=4u \tag{Refer equation \eqref{eq:eulerhomo}}
\end{align}
Since they asked for its reciprocal, the answer is $\frac{1}{4u}$







\item The value of $y$ at $x=0.3$ and $y\brak{x=0}=0$ for the differential equation $\frac{dy}{dx}=3y+2e^x$ is \underline{\hspace{1cm}}. (Rounded off to 3 decimal places)\hfill(GATE AG 2025)

\solution

\begin{enumerate}
\item
Let us define a function G(s) such that:
\begin{align}
    y&\system{L}G\brak{s}\\
    y'&\system{L}sG\brak{s} \tag{Refer \eqref{eq:laplace1d}}
\end{align}
Applying the Laplace transform:
 \begin{align}
     sG\brak{s}&=3G\brak{s}+2\mathcal{L}\{e^x\}\tag{From \eqref{eq:linearlaplace}}
 \end{align}

Now, we evaluate G(s)
\begin{align}
    G\brak{s}\brak{s-3}&=\frac{2}{s-1}\\
    G\brak{s}&=\frac{2}{\brak{s-1}\brak{s-3}}
   \end{align}
    \text{Using partial fractions:}
\begin{align}
    G\brak{s}&=\left(\frac{1}{s-3}-\frac{1}{s-1}\right)
\end{align}
Finding y using inverse Laplace transform:
\begin{align}
   y&\system{L}G\brak{s}
  \end{align}
From \eqref{eq:linearinverselaplace} and \eqref{eq:laplacee^ax},we can write:
\begin{align}
                 y&=e^{3x}-e^{x} 
\end{align}
finding y based on initial conditions:
\begin{align}
    x&=0.3\tag{given}\\
    y&=e^{3\times0.3}-e^{0.3}\\
    &=1.110
\end{align}
\item
Using forward euler method (see \eqref{eq:fwdeuler})to find the graph.The difference function of forward euler becomes:
\begin{align}
y_{n+1}=y_{n}+h(3y_n+2e^x)
\end{align}
Using it, the following graph is obtained: 
\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.5\textwidth]{figs/Euler.png}
    \caption{The plot depicting the divergence of forward euler from original at large values}
    \label{Forwardeuler}
\end{figure}
\end{enumerate}
The following code simulates the result:
\begin{lstlisting}
codes/diffeq.py
\end{lstlisting}
\end{enumerate}
%\end{document}